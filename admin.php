...
// CSRF: генерация токена для формы
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($birth_date)) {
        $validation_errors['birthdate'] = 'Дата рождения обязательна';
    } elseif (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $birth_date)) {
        $validation_errors['birthdate'] = 'Неверный формат даты';
    } else {
    ...
    if (empty($gender)) {
        ...
        $validation_errors['gender'] = 'Некорректное значение пола';
    }
    
    if (empty($selected_languages)) {
        $validation_errors['language'] = 'Выберите хотя бы один язык программирования';
    }
    ...
    if (!$contract_accepted) {
        $validation_errors['agreement'] = 'Подтвердите ознакомление с контрактом';
    }
    ...
    try {
        ...
        $validation_errors['database'] = 'Ошибка базы данных. Пожалуйста, попробуйте позже.';
        setcookie('form_errors', json_encode($validation_errors), 0, '/');
        setcookie('form_data', json_encode($_POST), 0, '/');
        header('Location: index.php');
        exit;
